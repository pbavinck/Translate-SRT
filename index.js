...
exports.translateSRTFiles = data => {
    const object = data;
    const file = storage.bucket(object.bucket).file(object.name);

    // Construct the filename of the output file
    const translatedFilename = path
        .parse(file.name)
        .base.replace(`-${SOURCE_LANGUAGE}`, `-${TARGET_LANGUAGE}`);
    let content;

    console.log(
        `Processing new text file: ${object.name} (${object.contentType})`
    );
    return file
        .download()
        .then(data => {
        ...
        .then(() => {
            let localFilename = `/tmp/${translatedFilename}`;
            console.log(
                `Uploading local file ${translatedFilename} to bucket ${TARGET_BUCKET}`
            );
            ...
            return storage
                ...
                .then(() => {
                    console.log(
                        `${localFilename} uploaded to ${TARGET_BUCKET}`
                    );
                })
                .catch(err => {
                    console.error(
                        'Error uploading translated file to bucket:',
                        err
                    );
                });
        })
